...
Youtube.prototype.clearAddPlayTrack = function (track) {
    ...
    ytdl.getInfo("https://youtube.com/watch?v=" + track.uri, {
        /* filter: "audioonly",  highWaterMark: 1024 * 1024 * 2*/
    }, function (err, info) {

        //  Choose format streaming
        var audioItems = info.formats
            //.filter(function (format) { return format.resolution === null })
            //.filter(function(format) { return format.resolution === '360p' })
            .filter(function (format) { return format.container === 'mp4' })            
            .sort(function (a, b) { return b.audioBitrate - a.audioBitrate })
        var audio = audioItems[0]

         // Choose format download
        var audioItemsDL = info.formats
            .filter(function (format) { return format.resolution === null })
            .sort(function (a, b) { return b.audioBitrate - a.audioBitrate })
        var audioDL = audioItemsDL[0]

        // Start download
        self.ytdown = function () {            
        ...
        if (err) {
            ...
            self.mpdPlugin.sendMpdCommand('stop', [])
                ...
                .then(function (values) {
                    return self.mpdPlugin.sendMpdCommand('load "' + audio.url + '"', []);
                })
                .fail(function (data) {
                    return self.mpdPlugin.sendMpdCommand('add "' + audio.url + '"', []);
                })
                .then(function () {
                            ...
                            var modalData = {
                                ...
                                buttons: [
                                    ...
                                    },
                                    {
                                        ...
                                        class: 'btn btn-info',
                                        emit: 'callMethod',
                                        payload: { 'endpoint': 'music_service/youtube', 'method': 'ytdown', 'data': {} }
                                    }
                                ]
                            };
...
Youtube.prototype.seek = function (position) {
    ...
    self.ytdown();
    self.commandRouter.pushToastMessage('info', 'Download this music');
    // End
